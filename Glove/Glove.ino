...
CircularBuffer<cap,1000> capture;
...
void loop() {
  ...
  if(fast){
    ...
    while(capture.remain()>0){
      capt=capture.pop();
      Serial.print(capt.mills);
      Serial.print(",");
      Serial.print(capt.event.acceleration.x); 
      Serial.print(",");
      Serial.print(capt.event.acceleration.y); 
      Serial.print(",");
      Serial.println(capt.event.acceleration.z);
    }
    digitalWrite(LED, LOW);  // if 1, switch LED Off
